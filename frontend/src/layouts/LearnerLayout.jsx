      ...
      <header className={`shrink-0 z-50 border-b backdrop-blur-md transition-colors duration-300 relative ${isDarkMode ? 'bg-[#212631]/90 border-[#323846] text-[#f1f3f9]' : 'bg-white border-slate-200 text-slate-900'}`}>
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-4 sm:gap-6">
              <Link to="/learner" className="flex items-center gap-2.5 font-bold tracking-tight text-lg">
                <span className="w-8 h-8 rounded-full bg-purple-600 text-white flex items-center justify-center font-mono text-xs font-bold shadow-md">tz</span>
                  ...
                  {NAV_LINKS.map((link) => {
                    ...
                    return (
                      <Link
                        key={link.to}
                        to={link.to}
                        className={`relative px-4 py-2 text-sm font-medium transition-all duration-300 rounded-full group ${isActive
                          ? isDarkMode
        ...
        {isMobileMenuOpen && (
          <div className={`md:hidden px-4 py-4 space-y-3 shadow-xl border-b ${isDarkMode ? 'bg-[#212631] text-[#f1f3f9] border-[#323846]' : 'bg-white text-slate-900 border-slate-200'}`}>
            ...
            <Link to="/learner/settings/notifications" onClick={handleLinkClick} className={`block px-4 py-2 rounded-full text-sm font-medium text-purple-400 ${isDarkMode ? 'hover:bg-[#2a3040]' : 'hover:bg-slate-100'}`}>Notification Settings</Link>
          </div>
        )}
      </header>
